...
public class Car : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // Добавляем детектор бордюров на каждое колесо
        foreach (var w in wheels)
        {
            if (w.wheelObject != null)
                w.wheelObject.AddComponent<WheelBordureDetector>();
        }
    }
    ...
    // Возвращает мировые позиции колёс
    // Порядок: 0=заднее правое, 1=переднее правое, 2=переднее левое, 3=заднее левое
    public Vector3 GetWheelPosition(int index)
    {
        if (index < 0 || index >= wheels.Length) return transform.position;
        if (wheels[index].wheelObject != null)
            return wheels[index].wheelObject.transform.position;
        return wheels[index].wheelWorldPosition;
    }
